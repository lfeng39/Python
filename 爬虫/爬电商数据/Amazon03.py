...
def amz():
    ...
    '''
    # 正则表达式
    re_AdHolder = r'<div\sdata-asin="(B.{9})"\sdata-index="(\d+)"\sdata-uuid=".*?".*?class=".*?(.{3})sg-col\s.+inner">\s+<.*?>\s+<.*?>\s+<.*?>\s+<div\sclass="s-card.*?><.+\ssrc="(.....).*?"\s.*?/></div></a></span></div><div.*?><div class="a-section.*?><div.*?><span.*?><a.*?><span.*?><span.*?>Sponsored</span></span><span.*?><span.*?>Sponsored</span></span>\s+<span.*?></span></a></span><div.*?><span>.*?</span><div.*?><span.*?><a.*?><span>.*?</span>\s+</a>\s+</span></div></div></div><h2.*?><a.*?><span.*?>(.{9}).*?</span>\s+</a>\s+</h2></div><div.*?><div.*?><.*?><span.*?><.*?>(.*?)<.*?><span.*?>(.*?)<.*?>.*?</.*?><.*?>.*?<.*?>.*?</.*?></span>.*?<.*?>.*?<.*?><.*?><.*?>(.*?)<.*?><.*?>(.*?)<.*?><.*?>.*?<.*?><.*?>.*?<.*?>.*?<.*?>(.*?)<.*?><.*?><.*?>.*?<.*?><span.*?>.*?<span.*?>.*?<.*?></.*?><.*?>.*?<.*?><.*?><.*?>.*?<.*?>.*?<.*?>(.*?)<.*?><.*?>.*?<.*?><.*?>.*?<.*?>.*?<.*?><.*?><.*?>(.*?)<'
