...
for k in range(len(cho_rows)):
    ...
    #补齐缺损字段
    new_row.insert(4, values_[4])
    new_row.insert(7, values_[7])
    new_row.insert(12, values_[12])
...
for i in range(1, len(new_df)):
    total_list.append(new_df[i][8])
print(sum(total_list))

x = len(new_df)
bbb = [x,'nan','','','/','/','/','/',sum(total_list),'/','/','/','/','/','/']
new_df.insert(0, bbb)
...
# create_xls_home.to_excel('C:/Users/explorer/Desktop/dd.xlsx')#★★★★★★这里是创建的表格路径及名称，可自己修改单引号中的内容★★★★★★

# print(create_xls_home.loc[:1])

create_xls_home.style.\
    to_excel('C:/Users/explorer/Desktop/dd.xlsx', engine='openpyxl')
